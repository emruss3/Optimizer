-- =====================================================
-- Deploy Enhanced Parcel Functions
-- Date: 2025-01-12
-- Description: Deploy enhanced get_parcel_at_point and get_parcel_by_id functions
-- =====================================================

-- First, ensure the zoning table has the new columns
-- (This should already be done by the schema enhancement migration)

-- Deploy enhanced get_parcel_at_point function
CREATE OR REPLACE FUNCTION public.get_parcel_at_point(
...
AS $$
  ...
  LEFT JOIN public.zoning z
    ON z.zoning_id = p.zoning_id  -- 👈 CORRECTED: Join on zoning_id
  CROSS JOIN click_pt
...
-- Deploy enhanced get_parcel_by_id function
CREATE OR REPLACE FUNCTION public.get_parcel_by_id(
  p_ogc_fid int
)
RETURNS TABLE (
  ogc_fid int,
