-- Beat-TestFit M1+M2 v1: multifamily/garden site-plan generator.
--
-- NOTE: already applied to the live database via MCP on 2026-07-10 (in three
-- iterations, consolidated here). Committed for version control — safe to
-- re-run (CREATE OR REPLACE), but the DB is not behind this file.
--
-- Grammar (matching the built-precedent references):
--   entry drive from the primary frontage -> full-height spine ->
--   bar rows on an oriented grid, alternating [parking street] and
--   [green court] between rows -> amenity pad nearest the entry ->
--   candidate persisted to siteplanner_session / siteplanner_candidate.
-- All construction in EPSG:2274 (US survey feet); areas are backend truth.
-- Output: 4326 GeoJSON for the live canvas + a translated local-feet frame.
-- Adaptive on tight sites: shrinks bar depth before dropping the parking
-- street, drops the street before dropping the plan.
--
-- v1 limitations (deliberate, tracked in docs/BEAT_TESTFIT_PLAN.md):
--   - entry = longest-frontage heuristic (real road edge lands with M1 data)
--   - uniform max-setback envelope (directional setbacks next)
--   - no density/FAR cap enforcement yet; parking shortfall is FLAGGED
--     (parking_below_ratio), balancing floors-vs-parking is the M5 objective
...
DECLARE
  ...
  spine geometry; spine_pad geometry;
  ...
  bars_u geometry;
  n_bars integer := 0; tot_fp numeric := 0; tot_park numeric := 0;
  tot_drive numeric := 0; tot_green numeric := 0; n_stalls integer := 0;
  gfa numeric; units integer; req_stalls integer;
  parcel_area numeric; flags jsonb;
  v_session uuid; v_candidate uuid; persisted boolean := true;
  basis text; ub geometry;
BEGIN
  ...
  IF lead_street THEN
    lead_park := ST_Difference(
      ST_Intersection(rot, ST_MakeEnvelope(bxmin, y, bxmax, y + stall_d, 2274)), spine_pad);
    IF lead_park IS NOT NULL AND NOT ST_IsEmpty(lead_park) THEN
      parks := lead_park;
    END IF;
    drive_band := ST_Intersection(rot, ST_MakeEnvelope(bxmin, y + stall_d, bxmax, y + stall_d + aisle, 2274));
      ...
      FOR comp IN SELECT (ST_Dump(band)).geom AS geom LOOP
        ...
        WHILE x + bar_min <= ST_XMax(comp.geom) - 2 LOOP
          ...
          WHILE try_len >= bar_min LOOP
            ...
            IF ST_Covers(rot, bar) AND NOT ST_Intersects(bar, spine_pad) THEN
    ...
    IF y + bar_depth + gap + bar_depth <= bymax THEN
      IF use_court THEN
        court_band := ST_Difference(
          ST_Intersection(rot, ST_MakeEnvelope(bxmin, y + bar_depth + 4, bxmax, y + bar_depth + gap - 4, 2274)), spine_pad);
      ...
      ELSE
        ...
        a1 := a0 + aisle;
        drive_band := ST_Intersection(rot, ST_MakeEnvelope(bxmin, a0, bxmax, a1, 2274));
        park_band := ST_Difference(
          ST_Intersection(rot, ST_MakeEnvelope(bxmin, y + bar_depth, bxmax, a0, 2274)), spine_pad);
        park_band := ST_Union(park_band, ST_Difference(
          ST_Intersection(rot, ST_MakeEnvelope(bxmin, a1, bxmax, a1 + stall_d, 2274)), spine_pad));
        IF drive_band IS NOT NULL AND NOT ST_IsEmpty(drive_band) THEN
  ...
  basis := format('%s garden bars · %s floors · entry drive from primary frontage · %s stalls surface (%s/unit target)',
                  n_bars, floors, n_stalls, ratio);
  ...
  BEGIN
    ...
    INSERT INTO public.siteplanner_candidate (session_id, typology, geometry_buildings, geometry_parking, geometry_drives, metrics)
