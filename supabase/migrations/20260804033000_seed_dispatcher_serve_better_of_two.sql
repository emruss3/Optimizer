-- Order-7 fix 3 (sweep 2026-08-04, parcel 320129): the dispatcher's
-- "capture < 55% → search core" rule assumed the search core always does
-- better on seed-weak parcels. It doesn't: 320129's seed placed 48.5% while
-- the search core returned 11.6% — the rule served the WORSE plan. The <55
-- branch now compares and serves the better of the two, never the worse.
-- (When the seed wins, the search candidate persisted during comparison
-- stays in the rail as honest history of the attempt.)
-- Everything else byte-identical to 20260804030000.
