...
-- Uses Regrid parcel use classifications and actual building footprint geometry
-- to create typology-specific built-form priors for the planner context.
...
begin
  ...
  select m.ogc_fid,m.lot_sqft_2274,m.zoning_subtype,m.usecode,m.usedesc,
         ...
         mod((degrees(case when m.d1>=m.d2 then m.a1 else m.a2 end)+180)::numeric,180)
  from measured m
