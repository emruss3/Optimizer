-- Subdivision generator v1 (Eric, 2026-09-03: "The design you came up with is
-- nothing like what our civil designed" … "Those [two MDHA sheets] should be
-- used as an example of how to design a neighborhood across any parcel of
-- size, not just this one.")
--
-- The two civil concepts are read as PRINCIPLES, not as a template for one
-- parcel. The generator derives the street network from the parcel's own
-- shape, in the parcel's oriented frame (x along the long axis, y across):
...
--      the short axis, and at what lot depth, is solved from the width: a
--      250-ft strip gets one spine at ~75-ft lots; a 1,200-ft tract gets a
--      grid of four.
--   2. CROSS CONNECTORS every ≤ p_max_block_ft when there are two or more
--      through-streets (ladder / grid) — no block longer than the max.
--   3. DOUBLE-LOADED lots fronting every street, REAR ALLEYS behind them
--      (garages off the alley), alleys at the outer edges too when the width
--      allows; front-loaded outer rows only when it does not (flagged).
--   4. COURTS interrupting the rows every p_court_every lots (both faces of a
--      street at the same station, so the court reads across the ROW).
--   5. AMENITY at the head of the site (the access end) when p_amenity_pct > 0
--      — the 08/30 concept's trade of lots for an amenity.
--   6. ACCESS from the unshared boundary at an end of the long axis; a
--      dead-end spine over 750 ft gets a cul-de-sac bulb at the far end
--      (Metro's dead-end limit); a side-only frontage gets an entrance
--      connector; nothing detected → through-access assumed and flagged.
--   7. WHOLE LOTS ONLY — a lot counts when its full width × depth sits inside
--      the parcel and clear of every ROW; every lot carries a buildable depth
--      after the district's front/rear setbacks.
-- Floodplain is NOT carved (no floodplain geometry layer in the DB); the
-- parcel-level FEMA fraction is reported as a flag so the count is never
-- mistaken for a net-of-floodplain yield. Frame: EPSG:2274 feet; every area
-- is measured by PostGIS, never estimated.

create or replace function public.fn_subdiv_world(
  g geometry, cs double precision, sn double precision, cx double precision, cy double precision)
returns geometry
language sql
immutable
as $$ select st_affine(g, cs, -sn, sn, cs, cx, cy) $$;

grant execute on function public.fn_subdiv_world(geometry, double precision, double precision, double precision, double precision) to anon, authenticated;
...
create or replace function public.fn_generate_subdivision(
  ...
  p_court_every integer default 12,
...
declare
  ...
  v_g geometry; v_area numeric; v_obb geometry; v_ring geometry; v_p1 geometry; v_p2 geometry; v_p3 geometry;
  ...
  v_strip geometry; v_piece geometry; v_px0 numeric; v_px1 numeric; v_t numeric; v_placed integer;
  ...
  v_front_loaded integer := 0; v_lot_no_alley boolean;
begin
  ...
  v_pl := st_affine(v_g, v_cs, v_sn, -v_sn, v_cs, -(v_cs*v_cx + v_sn*v_cy), (v_sn*v_cx - v_cs*v_cy));
  ...
  if v_n = 0 then
    return jsonb_build_object('parcel_ogc_fid', p_ogc_fid, 'generator_version', 'subdivision_v1',
      'error', 'parcel_too_narrow_for_a_street_and_a_lot',
  ...
  if v_n = 1 and not v_single_loaded then
    ...
    if v_bulb_start then
      ...
      -- a neighbour parcel across that end = a future through-connection (the 08/21 sheet's stub)
  ...
  for v_i in 1..v_n loop
    ...
    v_street_meta := v_street_meta || jsonb_build_object(
      ...
      'length_ft', round(st_length(v_ln)),
  ...
  if v_n >= 2 then
    ...
    for v_j in 1..array_length(v_cross_x, 1) loop
      ...
      v_street_len := v_street_len + st_length(v_ln);
      v_street_meta := v_street_meta || jsonb_build_object(
        'name', format('Cross %s', v_j), 'kind', 'cross', 'width_ft', v_row, 'length_ft', round(st_length(v_ln)),
        'geom_2274', st_asgeojson(public.fn_subdiv_world(v_str, v_cs, v_sn, v_cx, v_cy))::jsonb,
        'centerline_2274', st_asgeojson(public.fn_subdiv_world(v_ln, v_cs, v_sn, v_cx, v_cy))::jsonb);
    end loop;
  end if;
  ...
  -- 6. Amenity at the head of the site (replaces lot land; the streets run through)
  if p_amenity_pct > 0 then
    v_amenity_len := (p_amenity_pct/100.0 * v_area) / greatest(v_W - v_n*v_row, 50);
    if v_access = 'end' then
    ...
    v_amenity := st_difference(st_intersection(v_amenity, v_pl), v_row_union);
    v_amenity_area := st_area(v_amenity);
    v_flags := v_flags || to_jsonb(format('amenity_%s_pct_at_%s_head', p_amenity_pct, v_head));
  end if;

  -- 7. Lots on every street face, whole lots only; alleys behind; courts every N lots
  for v_i in 1..v_n loop
    for v_sgn in select unnest(case when v_single_loaded then array[1] else array[1,-1] end) loop
      ...
      v_has_alley := case when (v_i = 1 and v_sgn = -1) or (v_i = v_n and v_sgn = 1) then v_ea > 0 else true end;
      ...
      v_strip := st_difference(st_intersection(st_makeenvelope(-v_L/2 - 5, v_ylo, v_L/2 + 5, v_yhi, 2274), v_pl), v_row_union);
      ...
      for v_piece in select d.geom from st_dump(v_strip) d where st_area(d.geom) >= 0.5 * v_lot_w * v_dface loop
        ...
        v_t := v_px0; v_placed := 0;
        while v_t + v_lot_w <= v_px1 + 1e-6 loop
          v_court_here := p_court_every > 0 and v_placed > 0 and (v_placed % p_court_every) = 0
                          and (v_t + v_court_w) <= v_px1 + 1e-6;
          if v_court_here then
            ...
            if st_area(v_lotg) >= 0.9 * v_court_w * v_dface then
              ...
              if v_has_alley then
                v_alleys := v_alleys || st_difference(st_intersection(st_makeenvelope(v_t, v_alo, v_t + v_court_w, v_ahi, 2274), v_pl), v_row_union);
              end if;
              v_t := v_t + v_court_w; v_placed := 0;
              continue;
            end if;
            v_placed := 0;
          end if;
          ...
          v_lotg := st_intersection(v_rect, v_piece);
          -- A whole rectangle always counts. Against a slanted boundary an IRREGULAR lot
          -- counts when its front edge (the ROW side) is complete and it still holds ≥ 60%
          -- of the nominal area and the district minimum (requested SP widths: nominal only).
          ...
          if v_ok then
            v_nlots := v_nlots + 1; v_placed := v_placed + 1; v_a := st_area(v_lotg);
            ...
            if v_has_alley then
              v_alleys := v_alleys || st_difference(st_intersection(st_makeenvelope(v_t, v_alo, v_t + v_lot_w, v_ahi, 2274), v_pl), v_row_union);
            end if;
            v_t := v_t + v_lot_w;
          else
            v_t := v_t + greatest(v_lot_w/4, 5);   -- slide past a slanted edge or notch
          end if;
        end loop;
      end loop;
    end loop;
  ...
  -- Residual land (nothing assigned): perimeter reserve, slanted ends, leftover width
  ...
  v_residual := st_difference(v_pl, v_placed_union);
  ...
  -- Floodplain honesty: no geometry layer yet — report the parcel fraction
  ...
  if v_ae_pct > 0 then v_flags := v_flags || '"floodplain_not_carved_no_geometry_layer"'::jsonb; end if;

  v_basis := format('%s lots @ %s×%s ft (%s; depth %s) on %s %s-ft through-street%s%s · alleys %s ft · %s court%s · %s%% of land in ROW, %s%% in lots · buildable depth %s ft after %s/%s setbacks · access %s (%s) · generator subdivision_v1%s',
    ...
    v_alley, v_ncourts, case when v_ncourts = 1 then '' else 's' end,
    ...
    case when v_ae_pct > 0 then format(' · %s%% of the parcel is 100-yr floodplain (NOT carved — no geometry layer)', v_ae_pct) else '' end);

  return jsonb_build_object(
    'parcel_ogc_fid', p_ogc_fid, 'generator_version', 'subdivision_v1',
    ...
    'params', jsonb_build_object('lot_width_ft', p_lot_width_ft, 'lot_depth_ft', p_lot_depth_ft, 'row_width_ft', p_row_width_ft,
                                 'alley_width_ft', p_alley_width_ft, 'court_every', p_court_every, 'max_block_ft', p_max_block_ft,
                                 'amenity_pct', p_amenity_pct, 'access', p_access),
    'plan_basis', v_basis, 'flags', v_flags);
end
$function$;

grant execute on function public.fn_generate_subdivision(integer, numeric, numeric, numeric, numeric, integer, numeric, numeric, text) to anon, authenticated;
