-- Generic recovery for the remaining I-8 drive-network class.
--
-- Several large, irregular parcels produce drive polygons that all touch the
-- clipped frontage spine but remain topologically disconnected because the
-- spine itself is fragmented around notches/buildings. Search vertical and
-- horizontal side collectors plus short component links, reject building
-- crossings, and respect the remaining impervious budget. This runs before
-- final parking clipping and hard gates, so added pavement is fully recounted.
...
begin
  ...
  foreach axis in array array['vertical','horizontal'] loop
    ...
    foreach position in array positions loop
      ...
      corridor:=ST_CollectionExtract(
        ST_Intersection(
          p_envelope,
          ST_Buffer(links,half_width,'endcap=flat join=mitre')
        ),3);
      if obstacle is not null then
        corridor:=ST_CollectionExtract(ST_Difference(corridor,obstacle),3);
      end if;
      ...
      if obstacle is not null
         and coalesce(ST_Area(ST_Intersection(candidate,obstacle)),0)>1 then
        continue;
      end if;
...
begin
  ...
  if position('drive_components_connected_by_side_collector_v1' in d)>0 then
    return;
  end if;
  ...
  if position(old in d)=0 then
    raise exception 'I-8 side-collector insertion marker not found';
  end if;
  d:=replace(d,old,new);
  execute d;
end
...
comment on function public.fn_mf_connect_drive_components_collector(geometry,geometry,geometry,numeric,numeric) is
  'Obstacle-aware final collector for fragmented free-pack drive networks. Searches side/perimeter collectors, respects bars and impervious headroom, and runs before final hard gates.';
