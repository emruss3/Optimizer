...
begin
  ...
  create temp table if not exists _regrid_candidate_base_v2 (
    ogc_fid integer,
  ...
  truncate pg_temp._regrid_candidate_base_v2;

  with candidate_parcels as (
    select c.ogc_fid,c.ll_uuid::text as ll_uuid,c.lot_sqft_2274,c.usecode,c.usedesc,
           z2.zoning_subtype,public.st_distance(c.centroid_2274,v_centroid) as distance_ft,
    ...
    select c.*,
           ...
           count(*) over (partition by c.ogc_fid)::integer as building_count,
    ...
    join public.building_parcel_join j on j.ll_uuid=c.ll_uuid
    join public.buildings b on b.ed_bld_uuid=j.ed_bld_uuid
    where b.ed_bldg_footprint_sqft>0
  )
  insert into pg_temp._regrid_candidate_base_v2
  select ogc_fid,ed_bld_uuid,lot_sqft_2274,distance_ft,zoning_subtype,usecode,usedesc,use_class,
         public.fn_regrid_typology_match(p_typology,use_class),building_count,
         ed_bldg_footprint_sqft,total_footprint_sqft,
         100*total_footprint_sqft/nullif(lot_sqft_2274,0),
         case when ed_stories ~ '^[0-9]+([.][0-9]+)?$' then ed_stories::numeric else null end,
         gross_area_sqft
  from building_rows
  where rn=1;

  if not exists (select 1 from pg_temp._regrid_candidate_base_v2) then
    return jsonb_build_object('error','no nearby Regrid buildings');
  end if;
  ...
  loop
    foreach v_band in array array[0.5,1.0,2.0,999]::numeric[] loop
      ...
      from pg_temp._regrid_candidate_base_v2 c
  ...
  create temp table if not exists _regrid_selected_base_v2
    (like pg_temp._regrid_candidate_base_v2 including defaults) on commit drop;
  ...
  v_confidence:=case
    when v_mode='exact_same_zoning' and v_band<=1 and v_available_n>=p_min_comps then 'high'
    when v_mode in ('exact_same_zoning','exact_any_zoning','compatible_same_zoning') and v_available_n>=p_min_comps then 'medium'
  ...
  select jsonb_build_object(
    ...
    'provenance',jsonb_build_object('parcel_source','public.parcels usecode/usedesc','building_source','public.buildings Regrid footprint geometry and attributes','join','public.building_parcel_join','classifier','public.fn_regrid_use_class','matcher','public.fn_regrid_typology_match','sampling','up to 500 closest/lot-similar selected precedents')
  ) into v_result
...
revoke execute on function public.fn_regrid_use_class(text,text) from anon,authenticated;
revoke execute on function public.fn_regrid_typology_match(text,text) from anon,authenticated;
revoke execute on function public.fn_local_built_form_v2(integer,text,numeric,integer) from anon,authenticated;
grant execute on function public.fn_regrid_use_class(text,text) to service_role;
grant execute on function public.fn_regrid_typology_match(text,text) to service_role;
grant execute on function public.fn_local_built_form_v2(integer,text,numeric,integer) to service_role;

alter function public.fn_generate_mf_site_plan_v2(integer,text,integer,jsonb,uuid,boolean,uuid)
  set search_path = 'pg_catalog','public','extensions';

comment on function public.fn_local_built_form_v2(integer,text,numeric,integer) is
  'Optimized typology-aware Regrid built-form context. Selects use-compatible parcels before geometry normalization and caps deterministic geometry sampling at 500 precedents.';
