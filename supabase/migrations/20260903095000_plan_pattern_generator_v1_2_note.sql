-- The plan-pattern layer describes the subdivision generator it routes to.
-- v1.2 (20260903090000) stops the streets at the greenway unless through-
-- access is read, prices every crossing it takes, and pairs the courts at
-- the same station — the subdivision patterns' principles and the alignment
-- note say so now. fn_subdivision_calibration is unchanged (it reads whatever
-- vintage subdivision_sweep holds).
