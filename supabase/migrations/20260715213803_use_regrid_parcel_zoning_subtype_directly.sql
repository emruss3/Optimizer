...
--
-- Production reached this definition through several measured tuning patches.
-- This portable migration consolidates the final state so clean environments do
-- not replay the superseded implementations. Regrid parcel use is selected
-- before building geometry is loaded, the sample is deterministically capped,
-- and the parcel's own zoning_subtype avoids repeated planner_zoning lookups.
...
begin
  ...
  select p.zoning_subtype,p.centroid_2274,p.lot_sqft_2274
  ...
  loop
    foreach v_band in array array[0.5,1.0,2.0,999]::numeric[] loop
      ...
      where c.ogc_fid<>p_ogc_fid
        ...
        and (not v_same_zoning or c.zoning_subtype is not distinct from v_subtype)
  ...
  with selected_parcels as materialized (
    select c.ogc_fid,c.ll_uuid::text as ll_uuid,c.lot_sqft_2274 as lot_sqft,
           ...
           c.zoning_subtype,c.usecode,c.usedesc,
  ...
  select case when st.n<3 then jsonb_build_object('error','insufficient Regrid footprint geometries after normalization') else
    jsonb_build_object(
      ...
      'radius_ft',p_radius_ft,'sample_basis','current_regrid_building_stock','sample_cap',100,
      ...
      'selection',jsonb_build_object(
        ...
        'available_count',v_available_n,'sample_size',st.n,'sample_cap',100,
        'confidence',case
      ...
      'flags',jsonb_build_array('regrid_typology_selection_'||v_mode)
        ...
        || case when v_available_n>100 then jsonb_build_array('regrid_sample_capped_100') else '[]'::jsonb end,
      'provenance',jsonb_build_object(
        ...
        'sampling','mode selected in small indexed queries; up to 100 closest/lot-similar building geometries'
      )
    ) end
  into v_result
  from stats st;
...
comment on function public.fn_local_built_form_v2(integer,text,numeric,integer) is
  'Typology-aware Regrid context using parcels.zoning_subtype directly and at most 100 deterministic closest/lot-similar building geometries.';
