-- Emergency fix for get_parcels_in_bbox function
-- This removes any references to the non-existent p.parcel_id column
...
-- Create the fixed function without parcel_id references
...
begin
  ...
  from
    public.parcels p, env
  where
    ST_Intersects(p.wkb_geometry_4326, env.bbox)
    and p.sqft >= min_sqft
    and (cardinality(zoning_filter) = 0 or coalesce(p.zoning, p.zoning_code) = any(zoning_filter))
  limit max_results;
end;
$$;
