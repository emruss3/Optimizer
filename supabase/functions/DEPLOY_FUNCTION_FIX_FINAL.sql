-- =====================================================
-- DEPLOY: get_parcel_buildable_envelope Function Fix
-- Date: 2025-01-16
-- Description: Fix ambiguous column reference and add roads unique constraint
-- =====================================================

-- 1. Add unique constraint to roads table for ON CONFLICT support
ALTER TABLE public.roads 
ADD CONSTRAINT IF NOT EXISTS roads_osm_id_unique UNIQUE (osm_id);

-- Add index for better performance on osm_id lookups
CREATE INDEX IF NOT EXISTS idx_roads_osm_id ON public.roads(osm_id);

-- 2. Deploy the fixed get_parcel_buildable_envelope function
...
BEGIN
  ...
  SELECT geom, parcel_id INTO parcel_geom, parcel_geoid
  FROM planner_parcels
  WHERE parcel_id = p_ogc_fid::text
     OR parcel_id = (SELECT geoid::text FROM public.parcels WHERE ogc_fid = p_ogc_fid LIMIT 1)
  LIMIT 1;
