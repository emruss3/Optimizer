-- =====================================================
-- Enhanced get_parcel_by_id Function
-- Date: 2025-01-12
-- Description: Updated function with Regrid Standardized Zoning Schema fields
-- =====================================================

-- Drop the old version if it exists
DROP FUNCTION IF EXISTS public.get_parcel_by_id(int);

CREATE FUNCTION public.get_parcel_by_id(
  p_ogc_fid int
)
RETURNS TABLE (
  ogc_fid int,
...
AS $$
  ...
  LEFT JOIN public.zoning z
    ON z.zoning_id = p.zoning_id  -- 👈 Join on zoning_id
...
-- 5. This is the "fast path" function used by the optimized click handler
-- =====================================================
