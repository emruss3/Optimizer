...
const SiteWorkspace: React.FC<SiteWorkspaceProps> = ({ parcel }) => {
  ...
  // Track initial auto-generation: only runs once when the envelope first becomes available.
  // Reset when parcel changes so switching parcels triggers a fresh generation.
  ...
  // Reset auto-generation flag when parcel changes so each new parcel gets one auto-run.
  useEffect(() => {
  ...
  useEffect(() => {
    // Auto-generate ONCE when the envelope first becomes available for a parcel.
    // Deliberately NOT in the deps: config — config changes go through the manual
    // "Generate Plan" button only, so the optimizer doesn't re-run on every slider move.
    if (!hasValidGeometry || !envelopeMeters) return;
    if (hasAutoGeneratedRef.current) return; // already ran for this parcel
  ...
  }, [hasValidGeometry, envelopeMeters]); // intentionally excludes config and handleGenerate
