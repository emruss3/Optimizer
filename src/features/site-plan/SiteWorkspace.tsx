...
const SiteWorkspace: React.FC<SiteWorkspaceProps> = ({ parcel }) => {
  ...
  const handleGenerateCommercialPlate = useCallback(async (ctx: DesignContext) => {
    ...
    if (!envelopeMeters || !plannerCtxRef.current) {
      setPlanBasis('Commercial plate needs buildable envelope and context.');
      return;
    }
    ...
    if (!maxGfaSqft || maxGfaSqft <= 0) {
      setPlanBasis(`Commercial lot — ${ctx.zoningBase ?? 'zoning'} as-of-right · capacity from entitlement FAR · no allowable GFA`);
      return;
    }
